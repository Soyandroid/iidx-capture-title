    ...
    struct image *images, int nimages)
{
	...
	// TODO adjust this depending on the file you want to view
	// clip data starts at the beginning after a huge empty block
	const int ptr_data = 0x01BC;
	// TODO name data starts somewhere at the end
	const int ptr_names = 0x4ebc;

    clips = (struct clip *) (data + ptr_data);
    name = (char *) (data + ptr_names); 
    
    /* Guess how many clips there are with a heuristic */
    for (i = 0 ; i < nimages ; i++) {
		printf("%d %s, %X | pos: %d, %d, dim %d x %d\n", i, name, ptr_data + i * sizeof(struct clip), clips[i].x, clips[i].y, clips[i].w, clips[i].h);
...
/* Emit pile of TGAs */
    ...
    split_images(in_dir, out_dir, images, 200);
    fprintf(stderr, "(OK)\n\n");
